...
import TransactionTable from '@/app/components/staffcomponent/sales/TransactionTable'
...
export default function Sales() {
  ...
  const getTotalSalesInRange = (filteredTransactions, selectedKey, startDate, endDate, options) => {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((transaction) => {
      ...
      if(selectedKey === "today"){
        return transaction.date === date && transaction.employee_id === user.id;
      } else if(selectedKey === "date range"){
        return transactionDate >= start && transactionDate <= end && transaction.employee_id === user.id;
      } else{
        return transaction.employee_id === user.id
      }
    });
  }, [transactions, selectedKey, value.start, value.end, user]);
  

  const totals = useMemo(() => {
    const start = new Date(value.start);
    const end = new Date(value.end);
    return getTotalSalesInRange(filteredTransactions, selectedKey, start, end, options);
  }, [filteredTransactions, selectedKey, value.start, value.end, options]);
  ...
  return (
        <main className="flex flex-1 rounded-md flex-col gap-4 m-2 lg:gap-6 lg:m-4">
